...
const EXTRAS = [
  { no: '01', title: '初期費用', body: '乗り換えは一律10万円、新規立ち上げは立ち上げプラン30万円（物件タイプに応じたプロ撮影込み・全8工程）。移行作業・リスティング最適化（多言語）・運用セットアップを含みます。' },
...
const PRICING_FAQ = [
  {
    ...
    a: '毎月の基本料金は、変動運営委託費（売上の8%）＋固定管理費10,000円/物件/月です。初期費用は乗り換えが100,000円/物件、新規立ち上げが立ち上げプラン300,000円/物件（物件タイプに応じたプロ撮影込み・全8工程）。このほかは消耗品などの実費（実費＋20%）と、ご希望のときだけの有料オプションのみで、かかりうる費用は契約前にすべて金額つきで提示します。',
  },
  {
  ...
  },
  {
    ...
    a: '立ち上げプラン300,000円/物件（一括）に、プロ写真撮影・OTAリスティング初期設定（Airbnb・Booking）・Airbnb登録用電話番号の代行取得・料金初期設定（ダイナミックプライシング）・キーボックス設定と設備確認・消耗品/アメニティ初期管理・ハウスマニュアル作成（多言語）・清掃会社の手配と公開の全8工程を含みます。撮影は物件タイプ（間取り・広さ）に応じた内容で、一戸建・ヴィラなど大きな物件は事前見積もりのうえ差額をご案内します。撮影が不要な場合は80,000円割引。',
  },
  {
    q: '解約金はかかりますか？',
            ...
            <Reveal as="div" className="mt-12 sm:mt-16">
              <div className="flex flex-wrap items-end gap-x-6 gap-y-3">
                ...
                <div className="mb-1 sm:mb-2">
                  <p className="text-[15px] font-bold text-white/60">一括・全8工程</p>
                  <p className="mt-1 text-[14px] font-bold text-white">物件タイプに応じたプロ写真撮影込み</p>
                </div>
              </div>
            </Reveal>
