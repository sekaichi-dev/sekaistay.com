...
| **[@jiroisagame](https://x.com/jiroisagame)** | ジロー（本人） | ✅ 開設済（既存資産） | **約 40,000** | 要確認 |
| **@ss_unei_chan**（架空社員） | アバター運用 | ⚠️ 未作成 | — | 作成後にOAuth実行で自動投稿即有効化可能 |

> **戦略上の含意**: ジロー @jiroisagame は **すでに約4万人フォロワー** を保有する大型アカウント（ハンドル由来：ゲーム実況系コンテンツで形成された既存フォロワー基盤）。SEKAI STAY 民泊コンテンツへの転用は **既存フォロワー属性とのギャップ管理** が必須。短期的な KPI 目標は @tenichiliu 単独 + 新規獲得分でカウントし、@jiroisagame の既存4万人は **インプレッション資産** として扱うのが現実的
> 透明性: 架空社員 bio に「SEKAI STAY 運営チームメンバー」と明記（虚構ではなく実在運用チームのペルソナ化）
> @jiroisagame は自動投稿せず手動投稿運用（ジロー本人の既存アカウントで他コンテンツも混在するため）
...
> **前提**: @jiroisagame は約40,000 follower の既存資産（ゲーム実況系）として保有済み。下記 Combined Followers は **SEKAI STAY ブランドリーチに寄与する見込み分** として再設計（@tenichiliu 新規 + @ss_unei_chan + @jiroisagame の SEKAI STAY 関連投稿に反応する既存フォロワー比率推定）

| 指標 | M1（6月） | M2（7月） | M3（8月） |
|---|---|---|---|
| @tenichiliu Followers（新規構築） | 800 | 2,000 | 4,000 |
| @ss_unei_chan Followers（架空社員・新規） | 200 | 800 | 1,500 |
| @jiroisagame 民泊コンテンツ反応者（既存4万人の活性層） | 1,000 | 2,500 | 4,500 |
| 月間 Impressions（3アカウント合算・@jiroisagame の影響大） | 300K | 700K | 1.2M |
| LP 流入（X → /switch*） | 150 | 400 | 800 |
| X リード（org + paid） | 5-8 | 12-18 | 20-30 |
