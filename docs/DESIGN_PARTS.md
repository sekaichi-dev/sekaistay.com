...
- **PositionList（募集要項カード）**: `components/recruit/PositionList.tsx`（/recruit POSITIONS）。**CardGrid の募集要項版** — 画像(上・16:10・hover scale-110)に番号バッジと職種名を重ね、下に「雇用形態/勤務地」の帯・概要・「仕事内容 / 求める人物像」・末尾の応募リンクを積む。1件が複数ブロックを抱える要項カードで使う。
