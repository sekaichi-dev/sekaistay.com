...
- **PositionList（募集要項カード）**: `components/recruit/PositionList.tsx`（/recruit POSITIONS）。**NumberCards の明色背景版** — 背面ゴースト番号＋前面番号・hover で浮上＋上辺アクセント線という作りはそのままに、カードを paper＋rule にし、中に「雇用形態/勤務地」の帯と「仕事内容 / 求める人物像」の箇条書きを積む。1件が複数ブロックを抱える要項カードで使う。
